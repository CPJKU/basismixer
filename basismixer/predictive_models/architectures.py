    ...
    def __init__(
        ...
        if not isinstance(dropout, (list, tuple)):
            ...
            if len(dropout) != len(self.hidden_size):
                raise ValueError(
                    "`dropout` should be the same length " "as `hidden_size`."
                )
        ...
        if not isinstance(nonlinearity, (list, tuple)):
            ...
            if len(nonlinearity) != len(self.hidden_size):
                raise ValueError(
                    "`nonlinearity` should be the same length ", "as `hidden_size`."
                )
        ...
        self.output = nn.Linear(
            in_features=self.hidden_size[-1], out_features=self.output_size
        )
...
class RecurrentModel(NNModel):
    ...
    def __init__(
        ...
        dense_in_features = (
            self.recurrent_size * 2 if self.bidirectional else self.recurrent_size
        )
