    ...
    for piece, files in pair_files(folders, by_prefix=False).items():
        ...
        try:
            for match in files['match']:
                ...
                time_codec = pc.TimeCodec(normalization='beat_period_standardized')
                ...
                plot_targets(targets, perf_codec.parameter_names, '/tmp/{0}.png'.format(piece), onsets=onsets, xticks=measure_times, title=match)
